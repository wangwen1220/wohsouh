	...
	if (myeditor.mode=="wysiwyg"){
        ...
        if (!isPart) {
			...
			if(window.navigator.userAgent.toLowerCase().indexOf("msie") > 0) {
				...
				tmpDiv.innerHTML = tmpDiv.innerHTML.replace(/<p/gi,'<br /><p');
				tmpDiv.innerHTML = tmpDiv.innerHTML.replace(/<table/gi,'<br /><img');
				tmpDiv.innerHTML = tmpDiv.innerHTML.replace(/<img/gi,'<br /><img');
				tmpDiv.innerHTML = tmpDiv.innerHTML.replace(/<object/gi,'<br /><object');
			}
			...
			/*var strongarray = new Array();// 暂不处理strong和b标签，因为此标签中若包含图片元素，会把图片元素去除
			var strongcount = 0;
			for(var i=0;i<tmpDiv.getElementsByTagName('b').length;i++){
				...
				strongarray[strongcount] = tmpDiv.getElementsByTagName('b')[i].innerHTML.trim();
			...
			for(var i=0;i<tmpDiv.getElementsByTagName('strong').length;i++){
				...
				strongarray[strongcount] =tmpDiv.getElementsByTagName('strong')[i].innerHTML.trim();
			...
			//if(ie) html=html.replace(/#Format[A-Za-z]+_\d+#/gi,'');
...
function processFormatText(textContext) {
		...
		for (var i = 0; i < tmps.length; i++) {
			...
			html += f;
			if(tmp!="")
				html += "<p style='text-align:center;'>" + tmp + "</p>\n"
		}else{
			html += "<p>" + tmp + "</p>\n";
    	}
	  }
