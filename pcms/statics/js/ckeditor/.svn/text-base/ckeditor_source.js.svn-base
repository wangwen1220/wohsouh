		...
		function q(u, v, w, x) {
			...
			for (var A in x) {
				...
				if (y.filebrowser.action == 'Browse') {
					...
					if (C) {
						onchange_auto_upload = y.onClick = n;
						y.filebrowser.url = C;
						y.hidden = false;
					}
					...
					if (C) {
						var D = y.onClick;
						y.onClick = function(E) {//图片上传函数TODO
							alert('ok');
							var F = E.sender;
							if (D && D.call(F, E) === false) return false;
							if (onchange_auto_upload && onchange_auto_upload.call(F, E) === false) return false;
							return o.call(F, E);
						};
						y.filebrowser.url = C;
						y.hidden = false;
