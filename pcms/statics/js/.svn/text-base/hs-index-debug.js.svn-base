(function($){
	...
	$(function(){
		...
		// 设为首页
		$('#set-home').click(function(){$.setHome()});

		// 登录后加载用户菜单
		$.getJSON($login_bar.attr('data-url'), function(d) {
			$login_bar.html(d.html.replace('<script>(function($){$(function(){make_hoverbox("user_setting");});})(jQuery);</script>', ''));
		...
		$('.ui-tab').tabs({
			callback: function(){
				var $tab_cnt_item_cur = $(this).find('.ui-tab-cnt-item:not(.fn-hide)');
				if($tab_cnt_item_cur.has('img[data-src]')){
					$tab_cnt_item_cur.trigger('lazyload');
				}
			}
		}).find('.ui-tab-cnt-item.fn-hide').one('lazyload', function(){
			$('img', this).imglazyload({
				attr: 'data-src',
				fadeIn: true
			});
		}).end().find('.ui-tab-nav-item:first-child').addClass('first-child');
	...
	$(window).load(function(){
		// 页面加载完成后触发图片加载
		//var timer_vimg = setTimeout(function(){ $('img').trigger('sporty') }, 3000);
		//var timer_himg = setTimeout(function(){ $('.ui-tab-cnt-item.fn-hide').trigger('lazyload') }, 8000);
