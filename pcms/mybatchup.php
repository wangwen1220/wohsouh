...
$dir_file = "image";
$wpf=date('YmdHis');
//$save_path = getcwd() . '/upload1/image/'.$dir_file.'/';				// 保存的路径
$save_path = getcwd() . '/myshowpic/'.$dir_file.'/';				// 保存的路径
$upload_name = "Filedata";
//$max_file_size_in_bytes = 2147483647;				// 2GB in bytes 最大上传的文件大小为2G
$max_file_size_in_bytes = 20480000;
...
if(is_dir('myshowpic/'.$dir_file)){
	...
	if(!move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$fileName)) {
		...
		HandleError("suc:myshowpic/".$dir_file."/,".$fileName.",".$file_size);
		exit(0);
	}
}else{
	if(mkdir('myshowpic/'.$dir_file)){
