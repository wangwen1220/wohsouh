		...
		if ($arr['dosubmit']) {
			...
			foreach($id_arr as $id) {
				...
				foreach($ids as $catid) {
					...
					//content内容推送如global_id表
					$this->db->sselect ( "insert into v9_global_id (modeid,input_time,
							last_update_time,title,catid,username,thumb,url,islink)
							VALUES ('$modelid',
							'" . $r['inputtime'] . "','" . $r['updatetime'] . "',
							'" . $r['title'] . "',
							'" . $catid . "',
							'" . $r['username'] . "',
							'" . $r['thumb'] . "',
