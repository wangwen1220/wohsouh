...
?>
<form name="form1" action="?m=community&c=communityey&a=community_class_associated_label&typeid=<?php echo $classid?>" method="post">
<div class="pad-10">
	<div style="float:left; width:466px; padding:2px; margin-left:10px; margin-right:10px;">
	标签名：<input type="text" name="tag_name" value="" />
...
<div class="pad-10"> 
	<div style="float:left; width:466px; padding:2px; border:1px solid #d8d8d8; margin-left:10px; margin-top:10px; margin-right:10px;">
		<table width="100%" cellspacing="0" class="table-list">
			...
			<tbody id="load_catgory">
				<?php foreach($class_list as $r) { ?>
				<tr title="点击选择" onclick="select_list(this,'<?php echo $r['tag_name']?>',<?php echo $r['id']?>)">
	...
	<div style="overflow:hidden;_float:left;margin-top:10px;*margin-top:0;_margin-top:0">
		...
		<ul id="catname" class="list-dot-othors">
			<?php foreach($taglist as $r) { ?>
			<li><span class='old'><?php echo $r['tag_name']?></span></li>
			<?php }?>
		</ul>
		</fieldset>
	</div>
...
function select_list(obj,title,id,url) {
	if(!$(obj).hasClass('selected')){
		...
		var str = '<li id="'+sid+'" class="newadd"><input type="hidden" class="data" value=\'{"name": "'+title+'", "url": "'+url+'", "classid": "'+classid+'"}\' /><span>'+title+'</span><a href="javascript:;" class="close" onclick=\'remove_id("'+sid+'")\'></a></li>';
		$('#catname').append(str);
	}
}
...
$('#myform').submit(function(){
	...
	$.post(url + '&createclass=yes', {"data": data}, function(d){
		if(d == 1){
