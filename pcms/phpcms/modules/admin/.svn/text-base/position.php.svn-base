...
class position extends admin {
	protected  $db, $db_data, $db_content;
	...
	public function public_item() {	
		if(isset($_POST['dosubmit'])) {
			...
			$page = $_GET['page'] ? $_GET['page'] : '1';
			$pos_arr = $this->db_data->listinfo(array('posid'=>$posid,'siteid'=>$siteid),'listorder DESC', $page, $pagesize = 500);
			...
			foreach ($pos_arr as $_k => $_v) {
				...
				$r['catid'] = $_v['catid'];
				if($_v['modelid']==-1)//专题
					$r['url'] = "/index.php?m=special&c=special&a=edit&specialid=".$_v['id']."&menuid=868";
				else
					$r['url'] = go($_v['catid'], $_v['id']);
				$r['title'] = safe_replace($r['title']);
				$key = $r['modelid'].'-'.$r['id'];
				$infos[$key] = $r;
			}
	...
	protected function content_pos($id,$modelid) {
		$id = intval($id);
