...
class attachment {
	...
	public function __construct($module='', $catid = 0,$siteid = 0,$upload_dir = '') {
	...
	public function download($field, $value,$watermark = '0',$ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '')
	{
		...
		foreach($matches[3] as $matche)
		{
			...
			//如果是不是互联网的URL
			//或者URL中包含huoshow.com（本来就是火秀的图，不需要下载）
			/*if(strpos($matche, '://') === false || 
					strpos($matche, "huoshow.com") !== false) 
			{
				//content_input.class.php直接new attachment
				//导致MY_不能生效，直接改
				continue;
			}
			*/
