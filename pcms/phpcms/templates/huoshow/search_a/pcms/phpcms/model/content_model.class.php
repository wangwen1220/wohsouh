...
class content_model extends model {
	...
	public function status($ids = array(), $status = 99) {
		...
		if(is_array($ids) && !empty($ids)) {
			foreach($ids as $id) {
				...
				$this->sselect("update v9_global_id set status='$status' where id='$id'");
				$del = false;
				$r = $this->get_one(array('id'=>$id));
				if($status==0) {
				//退稿发送短消息、邮件
	...
	protected  function search_api($id = 0, $data = array(), $action = 'update') {
	...
	protected  function update_category_items($catid,$action = 'add',$cache = 0) {
