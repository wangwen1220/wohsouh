...
<script language="javascript" id='index_page' type="text/javascript" src="<?php echo JS_PATH?>huoshow_editor.js"></script>
...
<div id="content">
    ...
    <div class="col-auto mr8">
		<div id='js_position_control'></div>
    <div id='js_position' class="crumbs">
    <div class="shortcut cu-span"><a id='generate_home' href="?m=content&c=create_html&a=public_index&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('create_index')?></span></a><a id='update_cache' href="?m=admin&c=cache_all&a=init&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('update_backup')?></span></a><a href="javascript:art.dialog({id:'map',iframe:'?m=admin&c=index&a=public_map', title:'<?php echo L('background_map')?>', width:'700', height:'500', lock:true});void(0);"><span><?php echo L('background_map')?></span></a><?php echo runhook('admin_top_left_menu')?></div>
    <div id='current_pos_wrapper'><?php echo L('current_position')?><span id="current_pos"></span></div></div>
    	<div class="col-1">
        	<div class="content" id='js_content' style="position:relative; overflow:hidden">
                <iframe name="right" id="rightMain" class="right_main" src="?m=admin&c=index&a=public_main" frameborder="false" scrolling="auto" style="border:none; margin-bottom:30px" width="100%" height="auto" allowtransparency="true"></iframe>
                <!--<div class="fav-nav">
					<div id="panellist">
...
function wSize(){
	...
	//var heights = strs[0]-150,Body = $('body'); //去掉收藏条，修改iframe的高度
	var heights = strs[0]-120,Body = $('body');
	$('#rightMain, .right_main').height(heights);
	$('#js_content').height(heights+30);
	...
	$('#js_position_control').toggle(function(){
		$(this).addClass('down');
		$('#js_position').hide();
		$('#rightMain, .right_main').height(heights+28);
	}, function(){
		$(this).removeClass('down');
		$('#js_position').show();
		$('#rightMain, .right_main').height(heights);
	});
	if($('#js_position').is(':hidden')){
		$('#rightMain, .right_main').height(heights+28);
	}
	
	//var openClose = $("#rightMain, .right_main").height()+39; //去掉收藏条，修改iframe的高度
	var openClose = $("#rightMain, .right_main").height()+10;
...
function _M(menuid,targetUrl) {
	...
	//$("#rightMain, .right_main").attr('src', targetUrl);
...
function _MP(menuid,targetUrl) {
	...
	$("#rightMain, .right_main").attr('src', targetUrl+'&menuid='+menuid+'&pc_hash='+pc_hash);
