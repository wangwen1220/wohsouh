...
class content extends admin {
		...
		if($workflowid) {
			...
			//更改内容状态
                ...
                if($status==0)
                {
                    ...
                    {
                        foreach ($_POST['ids'] as $id) {
                            $urlarr = $this->db->sselect("SELECT url FROM v9_global_id WHERE id='$id'");
                            $url = $urlarr[0][url];
                            $n=0;
                            if(strpos($url,"http://")!==false)
                                $n=8;
                            $substr = substr($url,$n);
                            $path = substr($substr,strpos($substr,'/'));
                            $n = 500;
                            $path_arr = explode(".",$path);
                            for($i=1;$i<$n;$i++)
                            {
                                $pre = ($i==1)?"":"_".$i;
                                $file = $_SERVER["DOCUMENT_ROOT"].$path_arr[0].$pre.".".$path_arr[1];
                                if(file_exists($file))
                                    unlink($file);
                                else
                                    break;
		...
		if(!empty($this->categorys)) {
			foreach($this->categorys as $r) {
				...
				if($r['type']==1 || $from=='block') {
					...
					$r['type'] = 'init';
					$r['add_icon'] = "<a class='tab_link add_icon' target='right' href='?m=content&c=content&a=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."&pc_hash=".$_SESSION['pc_hash']."'><img src='".IMG_PATH."add_content.gif' alt='".L('add')."'></a> ";
				}
				$categorys[$r['catid']] = $r;
			}
		}
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					...
					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a class='tab_link' href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>\$catname</a></span>";
						$strs2 = "<span class='tab_link folder'>\$catname</span>";
						break;
				}
	...
	public function public_draftbox()
	{
...
//			echo $catid; 
			...
			foreach($this->categorys as $cid=>$r) {
				...
				//$r['disabled'] = ($modelid && $modelid != $r['modelid']) ? 'disabled' : '';
		...
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					...
					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a class='tab_link' href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid&pc_hash=".$_SESSION['pc_hash']."' target='right' onclick='open_list(this)'>\$catname</a></span>";
						break;
				}
