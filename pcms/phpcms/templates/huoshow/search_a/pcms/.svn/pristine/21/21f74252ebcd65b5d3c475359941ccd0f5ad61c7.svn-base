...
function tpl_cache($name,$times = 0) {
	...
	if(empty($info))
		return false;
	//如果某个进程正在写缓存文件，则读原的缓存文件
	$lock_file = $info["filepath"].$info["filename"].".lock";
...
function setcache($name, $data, $filepath='', $type='file', $config='', $timeout=0) {
	...
	//如果是读common下面的配置，则读memcache的内容，加快首页生成速度
	//if($filepath=="commons")
	//	$config = "shm";
	...
	$value = $cache->set($name, $data, $timeout, '', $filepath);
	
	if($filepath=='tpl_data')
	{
		unlink($filepath1.$filename.".lock");
	}
 ...
 */
function getcache($name, $filepath='', $type='', $config='') {
	//同一页面可能多次访问缓存，可直接缓存为变量
	//可以极大提高缓存效率高  by ck
	
	global $GLOBALVARS;
	if(($filepath=="commons" || 
		$filepath=="model") && !empty($GLOBALVARS['cache'][$name]))
		return $GLOBALVARS['cache'][$name];
	
	pc_base::load_sys_class('cache_factory','',0);
	//如果是写common下面的配置，则写入到memcache中去，加快首页生成速度
	//if($filepath=="commons")
	//	$config = "shm";
	...
	$value = $cache->get($name, '', '', $filepath);
	if($filepath=="commons" || 
		$filepath=="model")
	{
		$GLOBALVARS[cache][$name] = $value;
	}
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			if($curr_page==1) {
				...
				$multipage .= ' <a href="'.pageurl($urlrule, 1, $array).'">1</a><span class="ellipsis">...</span>';
			} else {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' <span class="ellipsis">...</span><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span class="current">'.$pages.'</span> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
		}
	}
	$multipage = str_replace('games/','',$multipage);
//	$multipage = str_replace('http://www.huoshow.com/games/','http://game.huoshow.com/',$multipage);
//	$multipage = str_replace('http://pnews.trunk.huoshow.com/games/','http://game.huoshow.com/',$multipage);
	return $multipage;
}
/**
 * 返回分页路径
...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'nopic.gif') {
	...
	//如果$imgurl第一个字符是/，说明是绝对定位
	//则不增加前缀
	//主要用于CSMTOP图片导入时要保持URL一致化
	//这里虽然也可以处理成upload和Uploadfile都指向到同一个目录，达到不修改系统源码的目的
	//但是由于为了保险起见，避免因某些原因，指向到同一目录后，PHPCMS可能和CMSTOP生成的图片重名
	//而互相替换,导致一些奇怪问题。
	...
	if(substr($imgurl, 0,1)=='/')
	{
		...
		//$tmp_str = trim($imgurl,"/");
		//$img_base_path = substr($tmp_str,0,strrpos($tmp_str,"/"));
		$upload_url = "";
		//$newimgurl = "/".dirname(trim($imgurl,"/")).'/thumb_hs_'.$width.'_'.$height.'_'.basename($imgurl_replace);
	}
	
	if(!file_exists($upload_path.$imgurl_replace)) 
		return IMG_PATH.$smallpic;
	
	$newimgurl = dirname($imgurl_replace).'/thumb_hs_'.$width.'_'.$height.'_'.basename($imgurl_replace);
	//如果已经存在同名的小图，则直接返回.
	//if(file_exists($upload_path.$newimgurl))
	//	return IMG_PATH.'thumb_hs_'.$width.'_'.$height.'_'.basename($imgurl_replace);
	...
	//echo "$width:$width_t\t$height:$height_t";die();
...
//	if ($catid == 9 || $catid == 10  || $catid == 11 || $catid == 12 || $catid == 13 || $catid == 14 || $catid == 109 || $catid == 131 ){
//		$symbol_arr = '';
//	}else{ 
//		$symbol_arr = $symbol;
//	}
	...
	foreach($arrparentid as $catid) {
		if ($category_arr[$catid]['catid'] == 83 || $category_arr[$catid]['catid'] == 84 || $category_arr[$catid]['catid']==85 || $category_arr[$catid]['catid']==86){
			$game = '首页';
		}else {
			$game = '';
		}
		...
		if ($category_arr[$catid]['catid'] == 237)
		$url = "http://live.huoshow.com";
		$pos .= '<a href="'.$url.'">'.$category_arr[$catid]['catname'].$game.'</a>'.$aa;
		$i++;
	}
	return $pos;
}
 ...
 */
function url_change($string){
	//游戏
	$string = str_replace('http://www.huoshow.com/games/','http://game.huoshow.com/',$string);
	//电影
	$string = str_replace('http://www.huoshow.com/movie/','http://movie.huoshow.com/',$string);
	//电视
	$string = str_replace('http://www.huoshow.com/tv/','http://tv.huoshow.com/',$string);
	return $string;
}
