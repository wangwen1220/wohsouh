...
class content_tag {
	...
	public function set_modelid($catid) {
		...
		//$this->category = getcache('category_content_'.$siteid,'commons','','memcache');
	...
	public function lists($data) {
		$catid = intval($data['catid']);
		//判断是否去掉推荐位里的信息
		if (intval($data['notid'])){
			$arr_data = implode(',',$data['notid']);
			$not_id = "AND id NOT IN ($arr_data)";
		}else {
			$not_id = '';
		}
		if(!$this->set_modelid($catid)) return false;
...
//			$id = intval($data['id']) ? " AND id != ".$data['id'] : '';
			if($this->category[$catid]['child']) {
				$catids_str = $this->category[$catid]['arrchildid'];
				$pos = strpos($catids_str,',')+1;
				$catids_str = substr($catids_str, $pos);
				$sql = "status=99 AND catid IN ($catids_str)".$thumb.$not_id;
			} else {
				$sql = "status=99 AND catid='$catid'".$thumb.$not_id;
			}
		}
		$order = $data['order'];
		...
		$content_output = new content_output($this->modelid,$catid,$this->category);
		foreach ($return as $key=>$value)
		{
			$return[$key] = $content_output->get($return[$key]);
		}
	 ...
	 * @param $data
	 */
	public function relation_pic($data) {
		$catid = intval($data['catid']);
		if(!$this->set_modelid($catid)) return false;
		if($this->category[$catid]['child']) {
			$catids_str = $this->category[$catid]['arrchildid'];
...
// 				$sql2 = $sql." AND `keywords` LIKE '%$_k%'".(isset($data['id']) && intval($data['id']) ? " AND `id` != '".abs(intval($data['id']))."'" : '');
// 				$r = $this->db->select($sql2, '*', $limit, '','','id');
				$sql2 = "SELECT * FROM v9_picture WHERE".$sql." AND `keywords` LIKE '%".new_addslashes($_k)."%'".(isset($data['id']) && intval($data['id']) ? " AND `id` != '".abs(intval($data['id']))."'" : '');
				$r = $this->db->sselect($sql2);
				$number += count($r);
				foreach ($r as $id=>$v) {
					if($i<= $data['limit'] && !in_array($id, $key_array)) $key_array[$id] = $v;
					$i++;
	...
	public function position($data) {
		...
		if(!empty($pos_arr)) {
			foreach ($pos_arr as $info) {
				...
				if($info['modelid']==-1)//推荐内容来自专题 by ck
				{
					$dbarr = $this->db->sselect("SELECT url FROM v9_special WHERE id='".$info['id']."'");
					$array[$key]['url'] = $dbarr[0]["url"];
				}
				else
				...
				if($this->set_modelid($info['catid']))
				{
					
					$content_output = new content_output($this->modelid,$catid,$this->category);
					
					$array[$key] = $content_output->get($array[$key]);
				}
