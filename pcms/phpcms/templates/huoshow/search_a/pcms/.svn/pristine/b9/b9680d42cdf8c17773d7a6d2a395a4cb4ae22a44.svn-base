 ...
 *
 * @copyright			(C) 2012 huoshow
 * @license				http://www.phpcms.cn/license/
 * @lastmodify			2012-2-13
 */

final class MY_mysqli {
	...
	public function connect() {
		...
		$this->link = new mysqli($this->config['hostname'], $this->config['username'],
		...
		$this->link->set_charset('utf8');
   		$this->link->select_db($this->config['database']);
	...
	public function fetch_next($type=MYSQL_ASSOC) {
		...
		if($type == MYSQL_ASSOC) $type = MYSQLI_ASSOC;
		if($type == MYSQL_NUM) $type = MYSQLI_NUM;
		if($type == MYSQL_BOTH) $type = MYSQLI_BOTH;
	...
	public function num_rows($sql) {
		...
		return $this->lastqueryid->num_rows;
	}

	public function num_fields($sql) {
		$this->lastqueryid = $this->execute($sql);
		//return mysql_num_fields($this->lastqueryid);
		return $this->lastqueryid->field_count;
	}

	public function result($sql, $row) {
		$this->lastqueryid = $this->execute($sql);
		$i=0;
		while (($row = $this->lastqueryid->fetch_row())===true) {
			if($row == $i)
				return $row[0];
			$i++;
		}
	...
	public function halt($message = '', $sql = '') {
		if($this->config['debug']) {
			$this->errormsg = "<b>MySQL Query : </b> $sql <br /><b> MySQL Error : </b>".$this->error()." <br /> <b>MySQL Errno : </b>".$this->errno()." <br /><b> Message : </b> $message <br /><a href='http://huoshow.com/help.php?errno=".$this->errno()."&msg=".urlencode($this->error())."' target='_blank' style='color:red'>Need Help?</a>";
			$msg = $this->errormsg;
