...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.indexOf("suc")!=-1) {
			...
			progress.setText("<a href=\"http://www.huoshow.com/"+fileinfo[0]+fileinfo[1]+"\" target=\"_blank\">"+fileinfo[1]+"</a>");
			var status = "恭喜你，文件上传成功！ <br />";
			status += "大小："+parseFloat(parseInt(fileinfo[2])/1024).toFixed(2)+ "K <br />";
			status += "地址：http://www.huoshow.com/"+fileinfo[0]+fileinfo[1];  //这里需要自己根据自己网站实际情况坐下修改
			progress.setStatus(status);
			progress.toggleCancel(false);
			document.getElementById('imglist').innerHTML += '[img]http://www.huoshow.com/'+fileinfo[0]+fileinfo[1]+'[/img]<br/>';
		}else{
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			progress.setError();
