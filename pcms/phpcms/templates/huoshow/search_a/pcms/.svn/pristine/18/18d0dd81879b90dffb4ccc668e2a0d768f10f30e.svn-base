...
class form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $allowuploadnum = '10') {
		...
		if($toolbar == 'basic') {
			...
			$toolbar = "['Undo','Redo','-','Bold','Italic','Underline','-','FontSize','-','TextColor','BGColor'],
				['Link','Unlink','Anchor','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock'],
				['Image','Flash','PageBreak','flvPlayer','autoformat','-','ToolbarCollapser'],
				'/',
				['Cut','Copy','Paste','PasteText','PasteFromWord','-','Print','-','Find','Replace','SelectAll','RemoveFormat'],
				['Strike','Subscript','Superscript','-','NumberedList','BulletedList','Outdent','Indent','Blockquote','ShowBlocks','-','Table','HorizontalRule','Smiley','SpecialChar'],
				'/',";
			if(defined('IN_ADMIN')) {
				$toolbar .= "['Source','-',";
			} else {
			...
			$toolbar .= "'Templates','-','Styles','Format','Font'],['attachment'],\r\n";
		} elseif($toolbar == 'desc') {
