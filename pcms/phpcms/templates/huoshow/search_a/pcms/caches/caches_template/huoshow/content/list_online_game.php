	...
	<div id='content'>
		...
		<div id='main'>
			<div class='ui-content-header'>
				<h2 class='title'><?php echo $CATEGORYS[$catid]['catname'];?>列表</h2>
                <div class='link'><?php $n=1;if(is_array(subcat($catid,0,0,$siteid))) foreach(subcat($catid,0,0,$siteid) AS $r) { ?><a href="<?php echo url_change($r[url]);?>"><?php echo $r['catname'];?></a> <?php $n++;}unset($n); ?></div>
                ...
                <!--<div class='link'><?php $n=1;if(is_array(subcat($catid,0,0,$siteid))) foreach(subcat($catid,0,0,$siteid) AS $r) { ?><a target="_blank" href='<?php echo $r['url'];?>'><?php echo $r['catname'];?></a> <?php $n++;}unset($n); ?></div>-->
			</div>
		...
		<div id='side'>
			<div class='ui-box-side ui-box-rdtj'>
				...
				<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
				<?php if($n==1 || $n==7) { ?>
				<h3 class='subtitle'><a target="_blank" href="<?php echo url_change($r[url]);?>" ><?php echo str_cut($r['title'],70,'');?></a></h3>
				<?php } else { ?>
			...
			<div class='ui-box-side ui-box-wydg'>
				...
				<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=4fa9544aa96261dcdce02e7f9f6fa160&action=position&posid=485&order=listorder+DESC&num=6\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'position')) {$data = $content_tag->position(array('posid'=>'485','order'=>'listorder DESC','limit'=>'6',));}?>
   				<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
					<li class='ui-img-list-item'>
						<a target="_blank" class='img' href="<?php echo url_change($r[url]);?>" ><img src='<?php echo thumb($r[thumb],122,91);?>' width='122' height='91' /></a>
						<a style="text-align:left;" target="_blank" class='title' href="<?php echo url_change($r[url]);?>" ><?php echo str_cut($r['title'],80,'');?></a>
					</li>
				<?php $n++;}unset($n); ?>
				<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
				</ul>
			...
			<div class='ui-box-side ui-box-zxphb'>
				...
				<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=43db320c2636e18d094d7009fcb6f0b0&action=rand_news_hothits&catids=253%2C263%2C272%2C286%2C304%2C310%2C311%2C313&newsnum=10\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'rand_news_hothits')) {$data = $content_tag->rand_news_hothits(array('catids'=>'253,263,272,286,304,310,311,313','newsnum'=>'10','limit'=>'20',));}?>
					<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
						<li class='ui-list-plus-item'><span class='num num-hot'><?php echo $n;?></span><a target="_blank" class='title' href="<?php echo url_change($r[url]);?>" ><?php echo str_cut($r['title'],60,'');?></a></li>
					<?php $n++;}unset($n); ?>
				<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
				</ol>
			</div>
