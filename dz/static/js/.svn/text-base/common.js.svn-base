            ...
            menuObj.style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + O + ')';
                    ...
                    if(!duration) {
                        setTimeout('hideMenu(\'' + menuid + '\', \'' + mtype + '\')', timeout);
                                ...
                                if(iframe){
                                    var frame = $('dx_menu_iframe_' + menuid);
                                    if(!frame){
                                        frame = document.createElement('iframe');
                                        frame.src = 'about:blank';
                                        frame.className = 'dx-menu-iframe';
                                        frame.id = 'dx_menu_iframe_' + menuid;
                                        frame.frameBorder = 0;
                                        frame.scrolling = 'no';
                                        frame.style.position = 'absolute';
                                        frame.style.cssText += [';left:0; top:0;',
                            ';width:', menuObj.clientWidth, 'px;', 'height:', menuObj.clientHeight, 'px;'].join('');
                                        menuObj.insertBefore(frame, menuObj.firstChild);
                                    }
                                }
}
...
function setMenuPosition(showid, menuid, pos) {
    ...
    var menuIframe = document.getElementById("dx_menu_iframe_" + menuid);
    if(menuIframe){
        menuIframe.style.cssText += [ ';width:', menuObj.clientWidth, 'px;',
            'height:', menuObj.clientHeight, 'px;'].join('');
    }
}
...
function showPrompt(ctrlid, evt, msg, timeout) {
    ...
    if(ctrlid) {
        ...
        showMenu({'mtype':'prompt','ctrlid':ctrlid,'evt':evt,'menuid':menuid,'pos':'210','duration':duration,'timeout':timeout,'zindex':JSMENU['zIndex']['prompt'], 'iframe': true});
        $(ctrlid).unselectable = false;
    } else {
        showMenu({'mtype':'prompt','pos':'00','menuid':menuid,'duration':duration,'timeout':timeout,'zindex':JSMENU['zIndex']['prompt'], 'iframe': true});
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt) {
    ...
    var s = '';
    s += '<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;</td><td class="m_c"><h3 class="flb"><em>';
    s += t ? t : '提示信息';
    ...
    showMenu({'mtype':'dialog','menuid':menuid,'duration':3,'pos':'00','zindex':JSMENU['zIndex']['dialog'],'cache':0,'cover':cover, 'iframe': true});
}
...
function showWindow(k, url, mode, cache, menuv) {
    ...
    var show = function() {
        ...
        v = {'mtype':'win','menuid':menuid,'duration':3,'pos':'00','zindex':JSMENU['zIndex']['win'],'drag':typeof drag == null ? '' : drag,'cache':cache, 'iframe': true};
        for(k in menuv) {
            v[k] = menuv[k];
        }
        showMenu(v);
    ...
    if(!menuObj) {
        ...
        menuObj.style.display = 'none';
        $('append_parent').appendChild(menuObj);
        menuObj.innerHTML = ['<table cellpadding="0" cellspacing="0" class="fwin">',
            '<tr><td class="t_l"></td><td class="t_c" ondblclick="hideWindow(\'', k, '\')"></td><td class="t_r"></td></tr>',
            '<tr><td class="m_l" ondblclick="hideWindow(\'', k, '\')">&nbsp;</td><td class="m_c" id="fwin_content_', k, '"></td><td class="m_r" ondblclick="hideWindow(\'', k, '\')"></td></tr>',
            '<tr><td class="b_l"></td><td class="b_c" ondblclick="hideWindow(\'', k, '\')"></td><td class="b_r"></td></tr>',
            '</table>'].join("");
        if (mode == 'html') {
...
function simulateSelect(selectId, widthvalue) {
    ...
    $(selectId + '_ctrl').onclick = function(e) {
        ...
        showMenu({'ctrlid':(selectId == 'loginfield' ? 'account' : selectId + '_ctrl'),'menuid':selectId + '_ctrl_menu','evt':'click','pos':'43', 'iframe': true});
        doane(e);
    };
