...
//今日必看
  /*if($act=='pages'){
	...
	//原创、音频、视频
		...
		//原创达人
		$query=DB::query("select uid,username from ".DB::table('video_field'). "  order by fanartnum desc limit 6");
	...
	$query=DB::query("select * from ".DB::table('video_list'). " where type=2  and auditstatus=2 and recommand=1 order by dateline desc limit 10 ");
		while($rs = DB::fetch($query)){
	...
	if($act=='yinpinqu'){
	//网友音频
		$query=DB::query("select * from ".DB::table('video_list'). " where type=1 and isfanart=1  and auditstatus=2 and recommand=1 order by dateline desc  limit 4 ");
		while($rs = DB::fetch($query)){
			$rs['title']=cutstr($rs['title'],16);
			$rs['avatar'] = avatar($rs['uid'], 'small');
			$hotaudio[]=$rs;
			...
			//die(json_encode($hotaudio));
				...
				//原创热榜
		$query=DB::query("select * from ".DB::table('video_list'). " where type=1 and auditstatus=2 and recommand=1 order by dateline DESC limit 0,20 ");
   ...
   if($act=='darenqu'){
   	//音乐达人
		$query=DB::query("select a.*,b.* from ".DB::table('video_field'). " as a left join ".DB::table('video_list'). " as b on a.audioid =b.myshowid order by a.weekaudio desc  limit 5");
		while($rs = DB::fetch($query)){
		...
		//视频达人
		$query=DB::query("select a.*,b.* from ".DB::table('video_field'). " as a left join ".DB::table('video_list'). " as b on a.videoid =b.myshowid order by a.weekaudio desc  limit 5");
   ...
   //最新发布
   if($act=='fabu'){
		$query=DB::query("select * from ".DB::table('video_list'). "  where auditstatus=2 order by dateline desc limit 5");
		while($rs = DB::fetch($query)){
			$rs['image']=ftpimg($rs['type'],$rs['isrecord'],$rs['videourl'],$rs['uid']);
