...
if($action == 'index') {
	...
	$staruserlist = $page < 2 ? groupuserlist($_G['fid'], 'lastupdate', 0, 0, array('level' => '3'), array('g.uid', 'g.username', 'g.level', 'joindateline', 'lastupdate')) : '';
	...
	if($_G['gp_op'] == 'group') {
		...
		if(!submitcheck('manageuser')) {
			...
			if(empty($_G['gp_srchuser'])) {
				$staruserlist = $page < 2 ? groupuserlist($_G['fid'], '', 0, 0, array('level' => '3'), array('m.uid', 'm.username', 'level', 'joindateline', 'lastupdate')) : '';
