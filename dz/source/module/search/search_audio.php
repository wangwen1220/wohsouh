...
if(!$_G['setting']['search']['audio']['status']) {
	showmessage('search_audio_closed');
}
...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		while($myshow = $sdb->fetch_array($query)) {
			$myshow['dateline'] = dgmdate($myshow['dateline']);//dgmdate($myshow['dateline']);
		...
		$multipage = multi($index['num'], $tpp, $page, "search.php?mod=audio&searchid=$searchid&orderby=$orderby&ascdesc=$ascdesc&searchsubmit=yes");

		$url_forward = 'search.php?mod=audio&'.$_SERVER['QUERY_STRING'];

		include template('search/myshow_search');

	} else {

		$searchstring = 'audio|title|'.addslashes($srchtxt);
		$searchindex = array('id' => 0, 'dateline' => '0');

		$query = $sdb->query("SELECT searchid, dateline,
			('".$_G['setting']['search']['audio']['searchctrl']."'<>'0' AND ".(empty($_G['uid']) ? "useip='$_G[clientip]'" : "uid='$_G[uid]'")." AND $_G[timestamp]-dateline<".$_G['setting']['search']['audio']['searchctrl'].") AS flood,
			(searchstring='$searchstring' AND expiration>'$_G[timestamp]') AS indexvalid
			FROM ".DB::table('common_searchindex')."
			WHERE srchmod='$srchmod' AND ('".$_G['setting']['search']['audio']['searchctrl']."'<>'0' AND ".(empty($_G['uid']) ? "useip='$_G[clientip]'" : "uid='$_G[uid]'")." AND $_G[timestamp]-dateline<".$_G['setting']['search']['audio']['searchctrl'].") OR (searchstring='$searchstring' AND expiration>'$_G[timestamp]')
			ORDER BY flood");
		...
		while($index = $sdb->fetch_array($query)) {
			if($index['indexvalid'] && $index['dateline'] > $searchindex['dateline']) {
				...
				showmessage('search_ctrl', 'search.php?mod=audio', array('searchctrl' => $_G['setting']['search']['audio']['searchctrl']));
			}
		}
		...
		if($searchindex['id']) {
			...
			if(!$srchtxt) {
				showmessage('search_invalid', 'search.php?mod=audio');
			}

			if($_G['adminid'] != '1' && $_G['setting']['search']['audio']['maxspm']) {
				if(($sdb->result_first("SELECT COUNT(*) FROM ".DB::table('common_searchindex')." WHERE srchmod='$srchmod' AND dateline>'$_G[timestamp]'-60")) >= $_G['setting']['search']['audio']['maxspm']) {
					showmessage('search_toomany', 'search.php?mod=audio', array('maxspm' => $_G['setting']['search']['audio']['maxspm']));
				}
			}

			$num = $ids = 0;
			$_G['setting']['search']['audio']['maxsearchresults'] = $_G['setting']['search']['audio']['maxsearchresults'] ? intval($_G['setting']['search']['audio']['maxsearchresults']) : 500;
			$srchtxtsql = addcslashes($srchtxt, '%_');
			$query = $sdb->query("SELECT myshowid FROM ".DB::table('video_list')." WHERE type=1 && auditstatus=2 && title LIKE '%$srchtxtsql%' ORDER BY myshowid DESC LIMIT ".$_G['setting']['search']['audio']['maxsearchresults']);
		...
		dheader("location: search.php?mod=audio&searchid=$searchid&searchsubmit=yes");
