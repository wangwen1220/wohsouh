...
function groupuserlist($fid, $orderby = '', $num = 0, $start = 0, $addwhere = '', $fieldarray = array(), $onlinemember = array()) {
	...
	$groupuserlist = array();
	$query = DB::query("SELECT $fieldadd,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('forum_groupuser')." g 
    LEFT JOIN ".DB::table('common_member')." m ON m.uid=g.uid 
	WHERE fid='$fid' $levelwhere $sqladd $orderby $limitsql");
...
function update_groupmoderators($fid) {
	...
	$moderators = groupuserlist($fid, 'level_join', 0, 0, array('level' => array('1', '2')), array('g.username', 'g.level'));
	if(!empty($moderators)) {
