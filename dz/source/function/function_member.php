...
function userlogin($username, $password, $questionid, $answer, $loginfield = 'username',$s_social_session='') {
	$return = array();
	...
	if($isuid == 3) {
		...
		if ($s_social_session){
			$return['ucresult'] = uc_user_login($username, $password, $isuid, 1, $questionid, $answer,$s_social_session);
		}else {
...
function ClearFail(){
	global $_G;
		DB::query("DELETE FROM ".DB::table('common_failedlogin')." WHERE ip='$_G[clientip]'", 'UNBUFFERED');
