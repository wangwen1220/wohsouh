...
function mkfeed($feed, $actors=array()) {
	...
	//+
	if ($feed['icon'] == 'audio' || $feed['icon'] == 'video') {
		$feed['title_template'] = str_replace('home.php?mod=myshow&do=myshow&item=done', 'home.php?mod=space&do=myshow', $feed['title_template']);;
	}
	//end
...
function feed_publish($id, $idtype, $add=0) {
	...
	switch ($idtype) {
		...
		case 'myshowid':
			//$plussql = $id>0?"p.picid='$id'":"p.uid='$_G[uid]' ORDER BY dateline DESC LIMIT 1";
			$query = DB::query("SELECT * FROM ".DB::table('video_list')." where myshowid='$id'");
			if($value = DB::fetch($query)) {
					if($value['type']==1){
						$setarr['icon'] = 'audio';
						$setarr['title_template'] = 'feed_myshow_audio';
					}else{
						$setarr['icon'] = 'video';
						$setarr['title_template'] = 'feed_myshow_video';
					}
					$setarr['id'] = $value['myshowid'];
					$setarr['idtype'] = $idtype;
					$setarr['uid'] = $value['uid'];
					$setarr['username'] = $value['author'];
					$setarr['dateline'] = $value['dateline'];
					//$setarr['target_ids'] = $value['target_ids'];
					//$setarr['friend'] = $value['friend'];
					$url = "home.php?mod=myshow&do=myshow&item=done&id=$id";
					$setarr['title_data'] = array('subject' => "<a href=\"$url\">$value[title]</a>");
					//$setarr['hot'] = $value['hits'];
					$status = 0;
					//$setarr['image_1'] = pic_get($value['filepath'], 'album', $value['thumb'], $value['remote']);
					//$setarr['image_1_link'] = $url;
					//$setarr['title_template'] = 'feed_pic_title';
