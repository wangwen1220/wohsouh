...
function showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '') {
	...
	if($type == 'radio') {
		...
		if ($setname == 'myshow_search_type') {
			...
			'<li><input class="radio" type="radio"'.($varnameid ? ' id="_v1_'.$varnameid.'"' : '').' name="'.$varname.'" value="1" '.$status.'>&nbsp;'.cplang('myshow_audio').'</li>'.
			'<li><input class="radio" type="radio"'.($varnameid ? ' id="_v0_'.$varnameid.'"' : '').' name="'.$varname.'" value="2" '.$status1.'>&nbsp;'.cplang('myshow_video').'</li>'.
			'</ul>';
		} else {//end
