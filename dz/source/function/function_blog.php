...
function blog_post($POST, $olds=array()) {
	...
	//引用保十洁接口
	require_once($_SERVER['DOCUMENT_ROOT']."/huoshow/FrameWork/BaoTenJie.class.php");
	$maxblogid = DB::result_first("SELECT MAX(blogid) AS blogid FROM pre_home_blog LIMIT 1");
	$param = array();
	//$task = 'Purify';	//用户自定义，Purify/Feedback/getNotify/getIndexResult
	$param['isBatch'] = 0;
	$param['appType'] = "blog";
	$param['textId'] = $maxblogid;
	$param['threadId'] = $maxblogid;
	$param['class'] = $_POST['classid'];
	$param['ip'] = $_G['clientip'];
	$param['userId'] = $_G['uid'];
	$param['author'] = $_G['username'];
	$param['title']= $_POST['subject'];
	$param['text'] = $_POST['message'];
	$param['pubDate'] = date('Y-m-d h:i:s',time());
//	print_r($param);
//  echo '<br>';
	$demo = BaoTenJie::textCheck($param,'Purify');
	$flag = $demo->flag;
	...
	if($olds['blogid']) {
		if ($flag == '0') {
			...
			showmessage('对不起，您提交的内容包含不良内容', '/home.php?mod=space&uid='.$_G['uid'].'&do=blog&view=me');
//			return false;
		}
		
	} else {
		if ($flag == '0') {//0为通过，1进入审核列表
			if($blogarr['catid']) {
				DB::query("UPDATE ".DB::table('home_blog_category')." SET num=num+1 WHERE catid='$blogarr[catid]'");
			}
