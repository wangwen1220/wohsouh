...
if($id) {
	...
	if($_GET['view'] == 'all') {
		...
		if($space['feedfriend']) {
			...
			$wheresql = "h.uid IN ($space[feedfriend])";
			...
			WHERE f.uid='$space[uid]' ORDER BY num DESC, dateline DESC LIMIT 0,500");
			while ($value = DB::fetch($query)) {
				$userlist[] = $value;
			}
		} else {
	...
	if($need_count) {
		...
		$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('home_album')." h
        LEFT JOIN ".DB::table('common_member')." m ON m.uid=h.uid 
		WHERE $wheresql"),0);

		if($count) {
			$query = DB::query("SELECT h.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('home_album')." h $f_index 
			LEFT JOIN ".DB::table('common_member')." m ON m.uid=h.uid 
						WHERE $wheresql ORDER BY updatetime DESC LIMIT $start,$perpage");
			while ($value = DB::fetch($query)) {
				if($value['friend'] != 4 && ckfriend($value['uid'], $value['friend'], $value['target_ids'])) {
					$value['pic'] = pic_cover_get($value['pic'], $value['picflag']);
				} elseif ($value['picnum']) {
