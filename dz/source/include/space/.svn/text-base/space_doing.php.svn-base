...
if(empty($count)) {
	$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('home_doing')." d
	LEFT JOIN ".DB::table('common_member')." m ON m.uid=d.uid
	WHERE $wheresql"), 0);
}
if($count) {
	$query = DB::query("SELECT d.*,if(m.nickname!='',m.nickname,m.username) as nickname  FROM ".DB::table('home_doing')." d $f_index
	LEFT JOIN ".DB::table('common_member')." m ON m.uid=d.uid
		WHERE $wheresql
...
if($doids) {
	...
	$values = array();
	$query = DB::query("SELECT d.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('home_docomment')." d FORCE INDEX(dateline) 
	LEFT JOIN ".DB::table('common_member')." m ON m.uid=d.uid 
	WHERE doid IN (".dimplode($doids).") ORDER BY dateline");
	while ($value = DB::fetch($query)) {
