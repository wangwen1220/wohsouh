...
if($id) {
	...
	if(!IS_ROBOT) {
		...
		if($_GET['view'] == 'all') {
			...
			if($space['feedfriend']) {
				$wheresql = "s.uid IN ($space[feedfriend])";
		...
		if($need_count) {
			$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('home_share')." s
			LEFT JOIN ".DB::table('common_member')." m ON m.uid=s.uid 
			WHERE $sharesql $wheresql"),0);
			if($count) {
				require_once libfile('function/share');
				$query = DB::query("SELECT s.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('home_share')." s $f_index
				LEFT JOIN ".DB::table('common_member')." m ON m.uid=s.uid 
