...
if($id) {
	...
	if($need_count) {
		...
		if($count) {
			$query = DB::query("SELECT bf.*, b.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('home_blog')." b $f_index
				LEFT JOIN ".DB::table('home_blogfield')." bf ON bf.blogid=b.blogid
                LEFT JOIN ".DB::table('common_member')." m ON m.uid=bf.uid 
				WHERE $wheresql
