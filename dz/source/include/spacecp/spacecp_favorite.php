...
if($_GET['op'] == 'delete') {
	...
	switch($type) {
		...
		case 'myshow';
			$idtype = 'myshowid';
			$title = DB::result_first('SELECT title FROM '.DB::table('video_list')." WHERE myshowid='$id' AND uid='$spaceuid'");
			$icon = '<img src="static/image/feed/myshow.png" alt="myshow" class="vm" /> ';
			break;
		//end
	}
