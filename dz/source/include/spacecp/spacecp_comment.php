...
if(submitcheck('commentsubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	switch ($idtype) {
		...
		//+
		case 'myshowid':
			$query = DB::query("SELECT b.*
				FROM ".DB::table('video_list')." as b WHERE b.myshowid='$id'");
			$myshowInfo = DB::fetch($query);
			if(empty($myshowInfo)) {
				showmessage('view_to_info_did_not_exist');
			}

			$tospace = getspace($myshowInfo['uid']);			

			$hotarr = array('myshowid', $myshowInfo['myshowid'], '');
			$stattype = 'myshowcomment';
			break;
		//end
		default:
	...
	switch ($_POST['idtype']) {
		...
		case 'myshowid':
			...
			if (!empty($myshowInfo)) {
				DB::query("UPDATE ".DB::table('video_list')." SET reply=reply+1 WHERE myshowid='$id'");
				
				$fs['title_template'] = 'feed_comment_myshow';
				$url = "home.php?mod=space&uid=$myshowInfo[uid]&do=myshow&id=$id";
				$fs['title_data'] = array('touser'=>"<a href=\"home.php?mod=space&uid=$myshowInfo[uid]\">".$myshowInfo['author']."</a>", 'subject' => "<a href=\"$url\">$myshowInfo[title]</a>");
				$fs['body_data'] = array();
			}
			break;
		//end
	...
	switch ($_POST['idtype']) {
		...
		//+
		case 'myshowid':
			$n_url = "home.php?mod=space&uid=$myshowInfo[uid]&do=myshow&id=$id&cid=$cid";

			$note_type = 'myshowcomment';
			$note = 'myshow_comment';
			$note_values = array('url'=>$n_url, 'subject'=>$myshowInfo['title']);
			$q_note = 'myshow_comment_reply';
			$q_values = array('url'=>$n_url);
	...
	//if(empty($comment)) {
	//+
	if(empty($comment) && $note_type != 'myshowcomment') {//end
	...
	//if($stattype) {
	//+
	if($stattype && $note_type != 'myshowcomment') {//end	
...
}elseif($_GET['op']=='ajaxreply'){ //ajax提交回复,并返回回复列表 
	include_once libfile('lang_template','language/home');
	if(empty($_G['uid'])){ //未登录
		echo -1;
		exit;
	}
	$query = DB::query("SELECT * FROM ".DB::table('home_comment')." WHERE cid='$cid' limit 1");
	if(!$comment = DB::fetch($query)) { //留言信息不存在
		echo -2;
		exit;
	}
	$_GET['rpv']=$bbcode->bbcode2html($_GET['rpv']);
	$setarr=array(
		'cid'=>$cid,
		'reply_content'=>$_GET['rpv'],
		'reply_uid'=>$_G['uid'],
		'reply_user'=>$_G['username'],
		'reply_dateline'=>$_G['timestamp'],
		'reply_ip'=>$_G['clientip'],
		'reply_stat'=>1,
	);
	DB::insert('home_comment_reply', $setarr, 1);
	//查询该留言所有的回复
	$query=DB::query("SELECT * FROM ".DB::table('home_comment_reply')." WHERE cid='$cid' and reply_stat=1");
	while ($value = DB::fetch($query)) {
		$value['pic_str']=$value['reply_user']?avatar($value['reply_uid'],'small'):'<img src="'.STATICURL.'image/magic/hidden.gif" alt="hidden" />';
		$value['user_show']=$value['reply_user']?'<a href="home.php?mod=space&uid='.$value['reply_uid'].'" class="c_tx q_namecard" target="_blank">'.$value['reply_user'].'</a>':$lang['anonymity'];
		$value['reply_dateline']=date('Y-m-d H:i:s',$value['reply_dateline']);
		$value['delete_str']=($_G['uid']==$value['reply_uid']||$_G['uid']==$comment['uid'])?'<a class="c_tx mod_comment_del" onclick="ajax.delelte_reply(\''.$value['reply_id'].'\');" style="cursor:pointer">'.$lang['delete'].'</a>':'';
		$list[] = $value;
	}
	echo json_encode($list);
	exit;
}elseif($_GET['op']=='delreplyajax'){ //ajax删除回复
	if(empty($_G['uid'])){ //未登录
		echo -1;
		exit;
	}
	$rpid=$_GET['rpid'];
	$reply_arr=DB::fetch_first('select a.reply_uid,b.uid from '.DB::table('home_comment_reply')." as a left join ".DB::table('home_comment')." as b on(a.cid=b.cid) where a.reply_id='$rpid' and a.reply_stat=1  limit 1");
	if(!$reply_arr){//回复信息不存在
		echo -2;
		exit;
	}
	if($reply_arr['reply_uid']!=$_G['uid']&&$_G['uid']!=$reply_arr['uid']){
		echo -3;
		exit;
	}
	DB::query('update '.DB::table('home_comment_reply')." set reply_stat=0 where reply_id='$rpid' limit 1");
    echo 1;
	exit;
