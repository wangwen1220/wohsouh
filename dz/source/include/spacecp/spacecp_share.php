...
if($_GET['op'] == 'delete') {
	...
	switch ($type) {
		case 'space':
			...
			$arr['body_data'] = array(
			'username' => "<a href=\"".DX_URL.$id."\">".$tospace['username']."</a>",
		...
		case 'myshow':
			...
			$feed_hash_data = "myshowid{$id}";

			$query = DB::query("SELECT b.* FROM ".DB::table('video_list')." as b WHERE b.myshowid='$id'");
			if(!$myshowInfo = DB::fetch($query)) {
				showmessage('blog_does_not_exist');
			}
			if($myshowInfo['uid'] == $space['uid']) {
				showmessage('share_not_self');
			}
			if($myshowInfo['friend']) {
				showmessage('logs_can_not_share');
			}
			if(isblacklist($myshowInfo['uid'])) {
				showmessage('is_blacklist');
			}

			$arr['title_template'] = lang('home/template', 'share_myshow');
			$arr['body_template'] = '<b>{title}</b>';
			$arr['body_data'] = array(
				'title' => "<a href=\"home.php?mod=space&uid=$myshowInfo[uid]&do=myshow&id=$myshowInfo[myshowid]\">$myshowInfo[title]</a>"
			);
			
			$note_uid = $myshowInfo['uid'];
			$note_message = 'share_myshow';
			$note_values = array(
				'url'=>"home.php?mod=space&uid=$myshowInfo[uid]&do=myshow&id=$myshowInfo[myshowid]",
				'title'=>$myshowInfo['title']
			);

			$hotarr = array('myshowid', $myshowInfo['myshowid'], $myshowInfo['hotuser']);
