...
//+
if(empty($click) || $click['available']==0) {
//end
//if(empty($click)) {
...
switch ($idtype) {
	...
	case 'myshowid':
		$idtype="myshowid";
		$sql = "SELECT b.* FROM ".DB::table('video_list')." b WHERE b.myshowid='$id'";
		$tablename = DB::table('video_list');
		break;
	//end
	default:
		$idtype = 'blogid';
...
if($_GET['op'] == 'add') {
	...
	if ($idtype == 'myshowid') {		
		if ($clickid == 5) {
			updatecreditbyaction('luguo', $item['uid']);
		} elseif ($clickid == 4) {
			updatecreditbyaction('leiren', $item['uid']);
		} elseif ($clickid == 3) {
			updatecreditbyaction('woshou', $item['uid']);
		} elseif ($clickid == 2) {
			updatecreditbyaction('xianhua', $item['uid']);
		} elseif ($clickid == 1) {
			updatecreditbyaction('jidan', $item['uid']);
		}
	}
	//end
