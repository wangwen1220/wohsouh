...
var icon_src = hl_plugin_path+'/bao10jie/calendar/';//图片的路径


/*-------------------------功能加强模块------------------------*/

//此函数直接打印出日历控件
function print_calendar_input(id, inp_name, inp_value){
	var obj = document.getElementById(id);
	var calendar_content = '<input name="'+inp_name+'" type="text" style="width:90px;height:13px;border:1px solid #6996B7;border-right:0" id="'+inp_name+'" value="'+inp_value+'" onClick="return showCalendar(\''+inp_name+'\', \'y-mm-dd\');" readonly >';
	calendar_content += '<img src="'+icon_src+'rili_bar.gif" align="absmiddle" id="calendar_icon_bar" onClick="showCalendar(\''+inp_name+'\', \'y-mm-dd\');" >';
	obj.innerHTML = calendar_content;
	return true;
}
...
function showCalendar(id, format) {
  ...
  if (calendar != null) {
    ...
    cal.setRange(2003, 2200);        // min/max year allowed.//更改起始年月
    cal.create();
  }
...
Calendar.cellClick = function(el) {
	...
	var now = new Date();	
   /*  if ( cal.date > now ) {Calendar.removeClass(el, "selected");Calendar.removeClass(el, "hilite");return false;}  //日期大于当天点击无效 */  
		newdate = true;
		// a date was clicked
	...
	if(el.navtype==2 ||el.navtype==-2||el.navtype==1||el.navtype==-1)newdate = false;
	if (newdate) {
		cal.callHandler();
	}
	if (closing) {
...
//	(this.weekNumbers) && ++title_length;
	...
	this._nav_py = hh("<img id=objyprev src=\""+icon_src+"yprev.gif\">", 1, -2);
...
//	this._nav_py = hh("&#x00ab;", 1, -2);
	...
	//	this._nav_ny = hh("&#x00bb;", 1, 2);
	this._nav_ny = hh("<img id=objynext src=\""+icon_src+"ynext.gif\">", 1, 2);
	this._nav_ny.width = 25;
	this._nav_ny.ttip = Calendar._TT["NEXT_YEAR"];

//	this._nav_pm = hh("‹", 1, -1);
	this._nav_pm = hh("<img id=objmprev src=\""+icon_src+"mprev.gif\">", 1, -1);
...
//	this._nav_nm = hh("›", 1, 1);
	this._nav_nm = hh("<img id=objmnext src=\""+icon_src+"mnext.gif\">", 1, 1);
...
//	this._nav_now.ttip = Calendar._TT["GO_TODAY"];
	...
	if (this.isPopup) {
		...
		var td_close = hh("<img id=objclose src=\""+icon_src+"close.gif\">", 1, 200);
...
Calendar.prototype.show = function () {
	...
	if (this.isPopup) {
		...
		this.tooltips.innerHTML = Calendar._TT["SEL_DATE"];
	}
	this.hideShowCovered();
};
...
Calendar._DN = new Array("日", "一", "二", "三", "四", "五", "六", "日");//zh
Calendar._MN = new Array("1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月");//zh
