...
if($_G['uid']) {
	...
	showmessage('login_succeed', '/', array('username' => $_G['member']['username'], 'uid' => $_G['uid']), array('extrajs' => $ucsynlogin));
...
if ($_G['gp_s_binding'] == 'binding' ) {
	$seccodecheck = 0;
	$secqaacheck = 0;
}elseif ($_G['gp_s_myshow'] == 's_myshow'){
	$seccodecheck = 0;
	$secqaacheck = 0;
}else {
...
$invite = getinvite();
if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck, $_G['gp_s_binding'], $_G['gp_s_myshow'])) {
	...
	if(!$activation) {
		...
		if($usernamelen < 3) {
			...
			showmessage('profile_username_allnumber');
		}
	...
	if(!$activation) {
//		if ($_G['gp_s_binding'] == 'binding') {//社会化绑定页面
...
//		$uid = uc_user_register($username, $password, $email, $questionid, $answer, $_G['clientip']);
	...
	DB::insert2('`huoshow_bbs_x2`.pre_common_member', $userdata);
	//插入绑定表
	if ($_G['gp_s_binding'] == 'binding'){
		$binding = array(
			'social_Uid' => $_G['gp_s_uid'],
			'uid' => $uid,
			'source' => $_G['gp_s_source']
		);
		DB::insert('community_bind',$binding);
	}
	$user_arr = array(
		'roomid' => $uid,
		'uid' => $uid,
	...
	DB::insert2('`huoshow_bbs_x2`.pre_common_member_count', $count_data);
	DB::insert('common_setting', array('skey' => 'lastmember', 'svalue' => $username), false, true);
	DB::insert2('`huoshow_bbs_x2`.pre_common_setting', array('skey' => 'lastmember', 'svalue' => $username), false, true);
