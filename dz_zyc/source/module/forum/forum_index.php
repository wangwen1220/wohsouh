...
header("location:http://bbs.huoshow.com/");
...
//最新回复
$sql = "SELECT * FROM ".DB::table('forum_thread')." WHERE replies > 0 ORDER BY lastpost DESC LIMIT 8";
$query = DB::query($sql);
while ($res = DB::fetch($query)) {
	$res['lastpost'] = date("Y-m-d",$res['lastpost']);
	$res['url'] = "thread-".$res['tid']."-1-1.html";
	$latestreply[] = $res;
}

//最新热贴
// $sql = "SELECT * FROM ".DB::table('forum_thread')." where displayorder>=0 ORDER BY dateline DESC LIMIT 8";
//最新精华贴
$sql = "SELECT * FROM ".DB::table('forum_thread')." where digest=1 ORDER BY dateline DESC LIMIT 8";
$query = DB::query($sql);
while ($res = DB::fetch($query)) {
	$res['url'] = "thread-".$res['tid']."-1-1.html";
	$latestretie[] = $res;
...
//$memcache->rm("forum_newimg");
$data = $memcache->get("forum_newimg");
if ($data !== false ) {
	$pic = $data;
} else {
	//创建图片目录
	$newImgDir = DISCUZ_ROOT.'data/attachment/forum/newimg/';
	if (!file_exists($newImgDir))	mkdir($newImgDir, 0777, 1 );
	//先删除之前的图片
	if(is_dir($newImgDir)){
        $dp = dir($newImgDir);
        while($fileName = $dp->read()){
        	@unlink($newImgDir.$fileName);
        }
        $dp->close();
	}
	//图片
	...
	foreach ($pic as $key=>$value) {	
		...
		$remotedir = DISCUZ_ROOT.'data/attachment/forum/'.$value['attachment'];
