...
function ftpimg($type=null,$record=null,$value=null,$id=null){
	if($type==1){
		$image="http://space.huoshow.com/static2/images/audio_default.png"; 
	}else{
		if($record==1){
...
function getuserbyuid($uid) {
	...
	if(empty($users[$uid])) {
		$users[$uid] = DB::fetch_first("SELECT m.*, mc.*, ms.* FROM ".DB::table('common_member')." m
			...
			WHERE m.uid='$uid'");
		//+
		$users[$uid]['showcoin'] = SIPHuoShowGetBalance($users[$uid]['uid']);
		//end
	}
	return $users[$uid];
}
...
function avatar($uid, $size = 'middle', $returnsrc = FALSE, $real = FALSE, $static = FALSE, $ucenterurl = '') {
	...
	if(!$staticavatar && !$static) {
        ...
        //'?'.filemtime($avatar)
        $avatar = DISCUZ_ROOT.'/uc_server/data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_cover'.'.jpg';
		$file = 'data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_cover'.'.jpg';
		//echo $file;die();
		if(file_exists(DISCUZ_ROOT.'/uc_server/'.$file)) {
			return $ucenterurl.'/'.$file.'?'.filemtime($avatar);
		} else {
			return $ucenterurl.'/images/no_cover_'.$size.'.gif';
		} 
	}
}

function avatar1($uid, $size = 'middle', $returnsrc = FALSE, $real = FALSE, $static = FALSE, $ucenterurl = '') {
	global $_G;
	static $staticavatar;
	if($staticavatar === null) {
		$staticavatar = $_G['setting']['avatarmethod'];
	...
	if(!$staticavatar && !$static) {
		...
		$dir3 = substr($uid, 5, 2);
		$avatar = DISCUZ_ROOT.'/uc_server/data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_cover'.'.jpg';
		$file = 'data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_avatar_'.$size.'.jpg';
		if(file_exists(DISCUZ_ROOT.'/uc_server/'.$file)) {
			return $ucenterurl.'/'.$file.'?'.filemtime($avatar);
		} else {
...
function submitcheck($var, $allowget = 0, $seccodecheck = 0, $secqaacheck = 0 , $s_binding = '', $s_myshow = '') {
	if(!getgpc($var)) {
		return FALSE;
	} else {
		global $_G;
		if ($s_binding =='binding') { //从绑定页申请过来，直接返回true
			return TRUE;
		}elseif ($s_myshow == 's_myshow'){
			return TRUE;
		}else {
...
function livepage($num, $perpage, $curpage, $mpurl,$ispage, $maxpages = 0, $page = 10, $autogoto = FALSE, $simple = FALSE) {
	global $_G;
	$ajaxtarget = !empty($_G['gp_ajaxtarget']) ? " ajaxtarget=\"".htmlspecialchars($_G['gp_ajaxtarget'])."\" " : '';

	$a_name = '&multilivepage='.$ispage;
	if(strpos($mpurl, '#') !== FALSE) {
		$a_strs = explode('#', $mpurl);
		$mpurl = $a_strs[0];
		$a_name = '#'.$a_strs[1];
...
function multilivepage($num, $perpage, $curpage, $mpurl,$ispage, $maxpages = 0, $page = 10, $autogoto = FALSE, $simple = FALSE) {
	global $_G;
	$ajaxtarget = !empty($_G['gp_ajaxtarget']) ? " ajaxtarget=\"".htmlspecialchars($_G['gp_ajaxtarget'])."\" " : '';
	...
	if($num > $perpage) {
		...
		$multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="'.$mpurl.'multilivepage=1'.$a_name.'" class="first"'.$ajaxtarget.'>首页</a>' : '').
		($curpage > 1 && !$simple ? '<a href="'.$mpurl.'multilivepage='.($curpage - 1).$a_name.'" class="prev"'.$ajaxtarget.'>'.$lang['prev'].'</a>' : '');
		for($i = $from; $i <= $to; $i++) {
			$multipage .= $i == $curpage ? '<strong>'.$i.'</strong>' :
			'<a href="'.$mpurl.'multilivepage='.$i.($ajaxtarget && $i == $pages && $autogoto ? '#' : $a_name).'"'.$ajaxtarget.'>'.$i.'</a>';
		}

		$multipage .= ($to < $pages ? '<a href="'.$mpurl.'multilivepage='.$pages.$a_name.'" class="last"'.$ajaxtarget.'>... '.$realpages.'</a>' : '').
		($curpage < $pages && !$simple ? '<a href="'.$mpurl.'multilivepage='.($curpage + 1).$a_name.'" class="nxt"'.$ajaxtarget.'>'.$lang['next'].'</a>' : '').
		($showkbd && !$simple && $pages > $page && !$ajaxtarget ? '<kbd><input type="text" name="custompage" size="3" onkeydown="if(event.keyCode==13) {window.location=\''.$mpurl.'multilivepage=\'+this.value; doane(event);}" /></kbd>' : '');
