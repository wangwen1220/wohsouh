...
function getuserdefaultdiy() {
	$defaultdiy = array(
			...
			'block' => array(
					'frame`frame1' => array(
							...
							'column`frame1_center' => array(
							        'block`video' => array('attr' => array('name'=>'video')),
									'block`audio' => array('attr' => array('name'=>'audio')),
			...
			'parameters' => array(
					...
					'audio'=>array('shownum' => 10),
					'video'=>array('shownum' => 10)
			)
	);
	return $defaultdiy;
}
...
function getblockhtml($blockname,$parameters = array()) {
	...
	switch ($blockname) {
		case 'profile':
			...
			$html .= "<h2><a href=\"home.php?mod=space&uid=$uid\" target=\"__blank\">".$space['nickname']."</a></h2>";
		...
		case 'statistic':
			...
			//得到进直播人数
			$sql = "SELECT SUM(max_members) con FROM ".DB::table('live_room_log')." WHERE uid=".$space['uid'];
			$visit = DB::fetch_first($sql);
			//得到MY秀作品的播放数
			$sql = "SELECT SUM(viewnum) viewnum FROM pre_video_list WHERE uid = ".$space['uid'];
			$mynumber = DB::fetch_first($sql);
			$sumnumber = $visit['con']+$mynumber['viewnum']+$space[views];
			$html .= '<p class="mbm xw1">';
			if(empty($parameters['banviews'])) $html .= lang('space', 'space_views', array('views' => $sumnumber ? $sumnumber : '--'));
		...
		case 'feed':
			...
			if(!IS_ROBOT && ckprivacy('feed', 'view')) {
				...
				$query = DB::query("SELECT f.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('home_feed')." f
				LEFT JOIN ".DB::table('common_member')." m ON m.uid=f.uid
				WHERE f.uid='$uid' ORDER BY dateline DESC LIMIT 0,$shownum");
				while ($value = DB::fetch($query)) {
					if(ckfriend($value['uid'], $value['friend'], $value['target_ids'])) {
		...
		case 'friend':
			...
			foreach ($friendlist as $key => $value) {
				...
				$html .= '<li><a href="home.php?mod=space&uid='.$value['fuid'].'" target="_blank"><em class="'.$classname.'"></em>'.avatar($value['fuid'],'small').'</a><p><a href="home.php?mod=space&uid='.$value[fuid].'" target="_blank">'.$value['nickname'].'</a></p></li>';
			}
		...
		case 'visitor':
			$do = 'friend';
			$view = 'visitor';
			$query = DB::query("SELECT v.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('home_visitor')." v
			LEFT JOIN ".DB::table('common_member')." m ON m.uid=v.vuid 
			WHERE v.uid='$uid' ORDER BY dateline DESC LIMIT 0,$shownum");
		...
		case 'wall':
			...
			if(ckprivacy('wall', 'view')) {
				...
				LEFT JOIN ".DB::table('common_member')." m ON m.uid=c.authorid
				WHERE c.id='$uid' AND c.idtype='uid' ORDER BY dateline DESC LIMIT 0,$shownum");
				while ($value = DB::fetch($query)) {
			...
			foreach ($walllist as $key => $value) {
				...
				if ($value['author']) {
					...
					$author = '<a href="home.php?mod=space&uid='.$value['authorid'].'" id="author_'.$value['cid'].'" target="_blank">'.$value['nickname'].'</a>';
				}else {
				...
				//updatetime:2011-03-19 begin
				$reply_str='';
				if($value['reply_arr']){
					foreach($value['reply_arr'] as $k=>$v){
						if ($v['reply_user']) {
							$reply_author_avatar = '<a href="home.php?mod=space&uid='.$v['reply_uid'].'" c="1" target="_blank" class="q_namecard">'.avatar($v['reply_uid'],'small').'</a>';
							$reply_author = '<a href="home.php?mod=space&uid='.$v['reply_uid'].'" class="c_tx q_namecard" target="_blank">'.$value['nickname'].'</a>';
						}else {
							$reply_author_avatar = '<img src="static/image/magic/hidden.gif" alt="hidden" />';
							$reply_author = lang('space', 'hidden_username');
						}
						$reply_replace_arr=array('reply_id'=>$v['reply_id'],'reply_usr_pic'=>$reply_author_avatar,'reply_user_a_str'=>$reply_author,'reply_content'=>$v['reply_content'],'reply_dateline'=>date('Y-m-d H:i:s',$v['reply_dateline']));
						$reply_str.=lang('space', 'wall_reply_str', $reply_replace_arr);
					}
				...
				$replacearr = array('author'=>$author, 'author_avatar' => $author_avatar, 'moderated' => $moderate_need, 'cid' => $value['cid'], 'message'=> $value['message'] , 'date' => $date, 'op'=> $op,'reply_str'=>$reply_str);
		...
		case 'music':
			...
			break;
		case 'video':
			if(ckprivacy('video', 'view')) {
				$videolist = array();
				$sql = "SELECT myshowid,uid,isrecord,title,videourl,dateline,hits,viewnum,isrecord,isfanart,timelength,temp_uid FROM ".DB::table('video_list')." WHERE uid='$uid'  AND `type`=2 AND auditstatus=2 ORDER BY dateline DESC LIMIT 0,$shownum";
				//echo $sql;
				$query = DB::query($sql);
				$html='<div class="content"><ul class="videolist imgtag cl">';
				while ($value = DB::fetch($query))
				{
					$html.='<li class="pv">';
					if($value['isfanart']=='1')
					{
						$html.='<div class="label"><i></i><em><span class="sc">'.$value['timelength'].'</span></em><em><span class="sc"></span>原创</em></div>';
					}
					else
					{
						$html.='<div class="label"><i></i><em><span class="sc">'.$value['timelength'].'</span></em><em><span class="sc"></span></em></div>';
					}
					if($value['isrecord']=='1')
					{
						//$imsrc="http://".FILE_HOST.'/saishi/'.$value['uid']."/".substr($value['videourl'],0,-4)."_small.jpg";
						$imsrc= cVideoPic($value, 'small');


						$html.='<a href="home.php?mod=space&do=myshow&id='.$value['myshowid'].'" target="_bank"><img src="'.$imsrc.'" width="96" height="72" onerror="this.onerror=null;this.src=\'static2/images/video_default.gif\'"></a>';
					}
					else
					{
						//$imsrc="http://".FILE_HOST.'/huoshow/video/'.$value['uid']."/".$value['videourl']."_small.jpg";
						$imsrc = cVideoPic($value, 'small');

						$html.='<a href="home.php?mod=space&do=myshow&id='.$value['myshowid'].'" target="_bank"><img src="'.$imsrc.'"width="96" height="72" onerror="this.onerror=null;this.src=\'static2/images/video_default.gif\'"></a>';
					}

					mb_strlen($value['title'],'utf-8') > 12 ? $title=mb_substr($value['title'],0,11,'utf-8').'...' :  $title=$value['title'];
					$html.='<span class="shipin"><a href="home.php?mod=space&do=myshow&id='.$value['myshowid'].'" target="_bank" title="'.$value['title'].'">'.$title.'</a></span><br />';
					$html.='<span class="videouploader">上传:'.date('m-d',$value['dateline']).'</span><br />';
					$html.='<span class="videouploader">播放:'.$value['viewnum'].'</span></li></li>';
				}
			}
			else 
			{
				$html .= lang('space','block_view_noperm');
			}
		...
		case 'audio':
			//$audiolist = array();
			if(ckprivacy('audio', 'view')) {
				$sql = "SELECT myshowid,singer,title FROM ".DB::table('video_list')." WHERE uid='$uid'  AND `type`=1 AND auditstatus=2 ORDER BY dateline DESC LIMIT 0,$shownum";
				//echo $sql;
				$query = DB::query($sql);
				$html='<div class="content"> <ul class="musiclist cl">';
				while ($value = DB::fetch($query))
				{
					mb_strlen($value['title'],'utf-8') > 6 ? $title=mb_substr($value['title'],0,5,'utf-8').'...' :  $title=$value['title'];
					mb_strlen($value['singer'],'utf-8') > 3 ? $singer=mb_substr($value['singer'],0,2,'utf-8').'...' :  $singer=$value['singer'];
					$html.='<li><span class="musicname" title="'.$value['title'].'"><a href="home.php?mod=space&do=myshow&id='.$value['myshowid'].'">'.$title.'</a></span><span class="singer" title="'.$value['singer'].'">'.$singer.'</span></li>';
				}
			}
			else 
			{
			...
			break;
		case 'zhiboblock':
			$html.='<div style="color:#000000;font-weight:bold;padding-top:5px;padding-bottom:5px;">直播状态</div>';
			$sql = "SELECT roomid FROM ".DB::table('live_room')." WHERE uid='$uid' AND stat = 1 ";
			$rs = DB::fetch_first($sql);
			if ($rs){
				$html.='<div style="text-align:center;"><a href="'.DX_HUOSHOW_SHOW_URL.$uid.'"><img src="/static2/images/widget_live_online.gif"/></a></div>';
			}else{
				$html.='<div style="color:#000000;text-align:center;"><a href="'.DX_HUOSHOW_SHOW_URL.$uid.'"><img src="/static2/images/widget_live_off.gif"/></a></div>';
			}
			$nowtime = time();
			$sql = "SELECT * FROM ".DB::table('live_notice')." WHERE uid='$uid' AND auditstatus=1 AND start_time>'$nowtime' ORDER BY start_time ASC LIMIT 0,1";
			
			$rst = DB::fetch_first($sql);
			
			if ($rst){
				$html.='<div style="color:#000000;font-weight:bold;padding-top:5px;padding-bottom:5px;">直播预告</div>';
				
				$sql = "SELECT * FROM ".DB::table('live_notice')." WHERE uid='$uid' AND auditstatus=1 AND start_time>'$nowtime' ORDER BY start_time ASC LIMIT 0,$shownum";
				
				$query = DB::query($sql);
				$html.='<div ><ul>';
				while ($value = DB::fetch($query)){
					$html.='<div ><li style="list-style: square;color:#3366AC;font-weight:bold">'.date("Y年m月d日 ",$value['start_time']).'<span style="color:orange;font-weight:bold">'.date("H:i",$value['start_time']).'</span></a></li>
					<div style="width:100%;padding-top:3px;padding-bottom:3px;margin-top:3px;margin-bottom:8px;color:#000000;">'.$value['content'].'</div></div>';
				}
				$html.='</ul></div>';
			}
