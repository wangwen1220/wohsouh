...
//-----请修改以下的模板，上面不需要改动-----
	'feed_blog_title_huoshow' => '<span class="act_actor">{actor}</span> 发表了新日志 <span class="act_subject">{subject}</span>',
	'feed_myshow_audio' => '<span class="act_actor">{actor}</span> 发表了新音频 <span class="act_audio">{subject}</span>',
	'feed_myshow_video' => '<span class="act_actor">{actor}</span> 发表了新视频 <span class="act_video">{subject}</span>',	
	'feed_comment_myshow' => '<span class="act_actor">{actor}</span> 评论了 {touser} 的作品 <span class="act_video">{subject}</span>',
	'feed_buyMagic' => '<span class="act_actor">{actor}</span> 购买了道具 ',
	'feed_createGroup' => '<span class="act_actor">{actor}</span> 创建了群组：<span class="act_group">{name}</span>',
	'feed_joinGroup' => '<span class="act_actor">{actor}</span> 加入了群组：<span class="act_group">{name}</span>',
	'feed_uploadImg' => '<span class="act_actor">{actor}</span> 上传了新图片',
	'feed_rankVote' => '<span class="act_actor">{actor}</span> 投了 <span class="act_touser">{touser}</span> 一票',
);
