...
if($_G['gp_view'] == 'all') {
	...
	if($viewtype == 'thread') {
		...
		require_once libfile('function/post');
		$query = DB::query("SELECT if(m.nickname!='',m.nickname,m.username) as nickname,p.authorid, p.tid, p.pid, p.fid, p.invisible, p.dateline, p.message, t.special, t.status, t.subject, t.digest,t.attachment, t.replies, t.views, t.lastposter, t.lastpost, t.displayorder FROM ".DB::table($posttable)." p
		INNER JOIN ".DB::table('forum_thread')." t ON t.tid=p.tid 
		LEFT JOIN ".DB::table('common_member')." m ON m.username=t.lastposter
		$threadsql
	...
	if($space['feedfriend']) {
		...
		$query = DB::query("SELECT f.*,if(m.nickname!='',m.nickname,m.username) as nickname  FROM ".DB::table('home_friend')." f
		LEFT JOIN ".DB::table('common_member')." m ON m.uid=f.fuid 
		WHERE f.uid='$_G[uid]' ORDER BY num DESC LIMIT 0,100");
		while ($value = DB::fetch($query)) {
			$userlist[] = $value;
		}
	} else {
...
if($need_count) {
	...
	if(!$havecache) {
		$query = DB::query("SELECT t.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('forum_thread')." t 
		LEFT JOIN ".DB::table('common_member')." m ON m.username=t.author 
