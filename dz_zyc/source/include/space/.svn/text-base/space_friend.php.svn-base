...
if($_GET['view'] == 'online') {
	...
	$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('common_session')." s $wheresql"), 0);
	if($count) {
		$query = DB::query("SELECT s.*,if(m.nickname!='',m.nickname,m.username) as nickname  FROM ".DB::table("common_session")." s	
        LEFT JOIN ".DB::table('common_member')." m ON m.uid=s.uid
		$wheresql AND invisible='0' ORDER BY lastactivity DESC LIMIT $start,$perpage");
...
if($fuids) {
	...
	$query = DB::query("SELECT cm.*, cmfh.*,if(cm.nickname!='',cm.nickname,cm.username) as nickname  FROM ".DB::table("common_member").' cm LEFT JOIN '.DB::table("common_member_field_home")." cmfh ON cmfh.uid=cm.uid WHERE cm.uid IN(".dimplode($fuids).")");
		while($value = DB::fetch($query)) {
		$value['isfriend'] = $value['uid']==$space['uid'] || $_G["home_friend_".$space['uid'].'_'.$value['uid']] ? 1 : 0;
