...
if($_GET['view'] == 'all') {
	...
	if($space['feedfriend']) {
		...
		WHERE f.uid='$space[uid]' ORDER BY num DESC LIMIT 0,100");
...
if($need_count) {
	...
	if($count) {
		...
		$query = DB::query("SELECT a.*, t.*,if(m.nickname!='',m.nickname,m.username) as nickname FROM ".DB::table('forum_thread')." t $apply_sql 
			INNER JOIN ".DB::table('forum_activity')." a ON $threadsql t.tid=a.tid
