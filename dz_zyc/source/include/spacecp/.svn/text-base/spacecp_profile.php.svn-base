...
if(submitcheck('profilesubmit')) {
	...
	if (isset($_POST['nickname'])){
		...
		updateNickName(trim($_POST['nickname']));
	}
	...
	if($setarr) {
		require_once  DISCUZ_ROOT.'./huoshow/FrameWork/DataBase.php';
		$dblink = new DataBase("");
		$dblink->query("update `huoshow_bbs_x2`.pre_common_member set password='".$setarr['password']."' where uid='".$_G['uid']."'");
		$dblink->query("update pre_common_member set password='".$setarr['password']."' where uid='".$_G['uid']."'");
		//DB::update2('`huoshow_bbs_x2`.pre_common_member', $setarr, array('uid' => $_G['uid']));
		$dblink->dbclose();
...
function updateNickName($nickname){
	global $_G;
	$uid= $_G['uid'];	
	$sql = "update ".DB::table('common_member')." SET nickname = '$nickname' WHERE uid = '$uid'";
	DB::query($sql);
	$sql = "update ".DB::table('ucenter_members')." SET nickname = '$nickname' WHERE uid = '$uid'";
	DB::query($sql);
	$sql = "update ".DB::table('weibo_msg')." SET nicename = '$nickname' WHERE uid = '$uid'";
	DB::query($sql);
