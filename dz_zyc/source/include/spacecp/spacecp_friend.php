...
if($op == 'add') {
	...
	if(friend_request_check($uid)) {
		...
		if(submitcheck('add2submit')) {
			...
			if(ckprivacy('friend', 'feed')) {
				...
				feed_add('friend', 'feed_friend_title', array('touser'=>"<a href=\"home.php?mod=space&uid=$tospace[uid]\">$tospace[nickname]</a>"));
			}
	...
	$query = DB::query("SELECT * FROM ".DB::table('home_specialuser')." ORDER BY displayorder ASC");
	while ($value = DB::fetch($query)) {
