...
class discuz_core {
	...
	function _init_env() {
		...
		$this->timezone_set();
        include_once DISCUZ_ROOT.'./source/function/function_newcore.php';
	...
	function _init_config() {
		...
		//+
		@include DISCUZ_ROOT.'./config/config_other.php';
		@include DISCUZ_ROOT.'./config/config_data.php';
		//end
		if(empty($_config)) {
...
class DB
{
	...
	function insert2($table, $data, $return_insert_id = false, $replace = false, $silent = false) {
	...
	function update2($table, $data, $condition, $unbuffered = false, $low_priority = false) {
		$sql = DB::implode_field_value($data);
		$cmd = "UPDATE ".($low_priority ? 'LOW_PRIORITY' : '');
		//$table = DB::table($table);
		$where = '';
