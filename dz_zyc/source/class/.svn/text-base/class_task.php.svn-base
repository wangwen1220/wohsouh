...
class task {
	...
	function tasklist($item) {
		...
		$tasklist = $endtaskids = $magicids = $medalids = $groupids = $moneyids = array();    //add $moneyids by Chris
		...
		if($moneyids) {    //add by Chris
			$query = DB::query("SELECT moneyid, name FROM ".DB::table('common_money')." WHERE moneyid IN (".dimplode($moneyids).")");
			while($money = DB::fetch($query)) {
				$this->listdata[$money['moneyid']] = $money['name'];
			}
		}
	...
	function view($id) {
		...
		switch($this->task['reward']) {
			case 'magic':
				...
				break;
			case 'money':    //add by Chris
				$this->task['rewardtext'] = DB::result_first("SELECT name FROM ".DB::table('common_money')." WHERE moneyid='".$this->task['prize']."'");
				break;
			case 'medal':
	...
	function draw($id) {
		...
		if($result === TRUE) {
			...
			if($this->task['reward']) {
				...
				} elseif($this->task['reward'] == 'money') {    //add by Chris
					$rewardtext = DB::result_first("SELECT name FROM ".DB::table('common_money')." WHERE moneyid='".$this->task['prize']."'");
	...
	function parter($id) {
		$query = DB::query("SELECT c.*,if(m.nickname!='',m.nickname,m.username) as nickname  FROM ".DB::table('common_mytask')." c
		LEFT JOIN ".DB::table('common_member')." m ON m.uid=c.uid
		WHERE taskid='$id' ORDER BY dateline DESC LIMIT 0, 8");
		$parterlist = array();
	...
	function reward() {
		switch($this->task['reward']) {
			...
			case 'money': return $this->reward_money($this->task['prize'], $this->task['bonus']); break;   //add by Chris
	...
	function reward_money($money, $amount) {    //add by Chris
		global $_G;
		SIPHuoShowUpdate($_G['uid'], $operation='TFD', $amount,$remark='活动：任务奖励');
	}
