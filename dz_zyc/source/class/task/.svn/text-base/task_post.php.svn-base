...
class task_post {
	...
	var $conditions = array(
		'act' => array(
			...
			'value' => array(
				...
				array('bestpost', 'post_complete_var_act_bestpost'),
			),
			'default' => 'newthread',
			'sort' => 'complete',
		),
	...
	function csc($task = array()) {
		...
		if($taskvars['act']) {
			...
			}elseif($taskvars['act'] == 'bestpost') {
				$tbladd .= ", ".DB::table('forum_thread')." t";
				$sqladd .= " AND p.first='1' AND t.digest>'0' and p.tid=t.tid";
			}
		}
	...
	function view($task, $taskvars) {
		...
		if($taskvars['complete']['act']['value'] == 'newreply') {
			...
			} elseif($taskvars['complete']['act']['value'] == 'bestpost') {
				$return .= lang('task/post', 'task_complete_act_bestpost', array('num' => $taskvars['complete']['num']['value']));
			}else {
