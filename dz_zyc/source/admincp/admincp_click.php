...
if(empty($operation)) {
	$idtype = in_array($_G['gp_idtype'], array('blogid', 'picid', 'aid', 'myshowid')) ? trim($_G['gp_idtype']) : 'blogid';
	if(!submitcheck('clicksubmit')) {
		...
		showsubmenu('nav_click', array(
			...
			#+
			array('click_edit_myshowid', 'click&idtype=myshowid', $idtype == 'myshowid' ? 1 : 0),
			#end
		));
