...
function uc_user_login($username, $password, $isuid = 0, $checkques = 0, $questionid = '', $answer = '', $s_social_session = '') {
	$isuid = intval($isuid);
	$return = call_user_func(UC_API_FUNC, 'user', 'login', array('username'=>$username, 'password'=>$password, 'isuid'=>$isuid, 'checkques'=>$checkques, 'questionid'=>$questionid, 'answer'=>$answer, 's_social_sess'=>$s_social_session));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
