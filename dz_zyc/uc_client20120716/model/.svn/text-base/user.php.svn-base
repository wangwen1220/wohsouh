...
class usermodel {
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		$uid = $this->getHuoshowNo();
		if (empty($uid)) {
			return -7;//分配火秀号出错
		} else {
			$this->db->query("UPDATE ".UC_DBTABLEPRE."huoshowno SET status=1 AND usedateline='$time' WHERE id='$uid'");
			...
			$time = time();
			$this->db->query("UPDATE ".UC_DBTABLEPRE."huoshowno SET status=1,usedateline='$time' WHERE id='$uid'");
	...
	function getHuoshowNo() {
		//获得当前用户可分配的ID总数
		$sql = "SELECT count(*) as idcount FROM pre_ucenter_huoshowno WHERE level=1 AND status=0 AND id > 112063";
		$uid_count_arr = DB::fetch_first($sql);
		$user_count = $uid_count_arr['idcount'];
		if($user_count<=0){
			//没有可分配的用户ID
			showmessage('profile_dontuse_uid_err');
		}
		$huo_rand = rand(1,$user_count);
		$sql = "SELECT id FROM pre_ucenter_huoshowno WHERE level=1 AND status=0 AND id > 112063 order by id asc limit $huo_rand,1";
		$uid_arr = DB::fetch_first($sql);
		return $uid_arr['id'];
		
		//获得火秀号
//		return $this->db->result_first("SELECT id FROM ".UC_DBTABLEPRE."huoshowno WHERE level=1 AND status=0 ORDER BY id ASC LIMIT 1");
	}
