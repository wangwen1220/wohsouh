...
class PagerSplit {
	...
	public function __construct($p_totalNum, $p_pageIndex, 
		 $p_pageSize = 10,$linkType=false,$p_initNum=10,$css="page.css") 
	{
		...
		$this->mReturnVal = "<link href=\"/huoshow/css/$css\" rel=\"stylesheet\" type=\"text/css\" />";
...
//{$this->CurrentUrl}={$this->TotalPages}
	public function GetPagerContent() {
		...
		}else
		{
			...
			{
				$str .="<a href='".str_replace("{#page}",1,$this->pageUrl)."' class='tips' title='首页'>首页</a> "."\n";
		...
		if($this->totalPagesCount-$this->pageIndex>$this->initNum/2 && $this->totalPagesCount>$this->initNum)
		{
			...
			{    $currnt="";    }
			if(empty($this->mLinkType))
				$str .="<a href='{$this->pageUrl}={$this->totalPagesCount} ' {$currnt}>...{$this->totalPagesCount}</a>"."\n";
			else
				$str .="<a href='".str_replace("{#page}",$i,$this->pageUrl)."' {$currnt}>...{$this->totalPagesCount}</a>"."\n";
		}
		...
		*/
		...
		}else
		{
			...
			else 
			{
				...
				$str .="<a href='".str_replace("{#page}",$this->totalPagesCount,$this->pageUrl)."' class='tips' title='末页'>末页</a> "."\n" ;
			}
		}		
