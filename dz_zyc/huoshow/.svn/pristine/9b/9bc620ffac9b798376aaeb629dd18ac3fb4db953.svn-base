...
class Controller_AvatarFlashUpload extends Controller_Base{
    ...
    public function showuploadAction() {
        ...
        $uid = $this->config->uid;
        $avatardir  = $this->config->avatardir;
        //var_dump($uid);
        if( $uid === null || $uid == 0 ){
            return -1;
        }
        ...
        $uc_avatarflash = $this->config->uc_api.'/images/camera.swf?ccc=aaa&nt=1&inajax=1&input='.$uc_input.'&agent='.md5($_SERVER['HTTP_USER_AGENT']).'&ucapi='.urlencode($this->config->uc_api. substr( $_SERVER['PHP_SELF'], strrpos($_SERVER['PHP_SELF'], '/') ) ).'&uploadSize='.$this->config->uploadsize;
        if( $returnhtml == 1 ) {
            $result = '<object id="mycamera" data="'.$uc_avatarflash.'" type="application/x-shockwave-flash" style="width:450px; height:250px;">
    ...
    function rectavatarAction() {
        ...
        $avatardir_root = $this->input('avatardir');
        $this->config->avatardir = $avatardir_root;
        ...
        //$avatarrealdir  = realpath( $avatardir_root. DIRECTORY_SEPARATOR . $avatarpath );
        //var_dump( $avatardir_root. DIRECTORY_SEPARATOR . $avatarpath);die();
        if(!is_dir( $avatarrealdir )) {
            $this->make_avatar_path( $uid, realpath($this->config->avatardir) );
        }
        //var_dump($avatarrealdir);
        $avatartype = common::getgpc('avatartype', 'G') == 'real' ? 'real' : 'virtual';
        
        $avatarsize = array( 1 => 'big', 2 => 'middle', 3 => 'small');
        
        $success = 1;
        //var_dump(realpath( $this->config->avatardir));die();
