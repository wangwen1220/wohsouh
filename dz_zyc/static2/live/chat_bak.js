...
function receiveMessage(o) {
    ...
    if(o.type == "1") {
        ...
        m.push('<span class="m-msg">', o.msg, '</span>');
        m.push('</div>');
        chat.log(m.join(''));
    }else{
...
/*更新当前用户的余额*/
function updateCoin(o) {
    HUI.printCallInfo();
    o = hs.parseJSON(o);
    if(o.huocoin) {
        chat.huocoin = o.huocoin;
    }
}
...
var chat = {
    ...
    makeUserMenu: function (u) {
        ...
        if(u.usertype<'5') {
            if(u.cansay == "1") {
                ul.push('<a href="javascript:;" onclick="_mo.cantSpeak(\'', u.uid,
                    '\');chat.closeUserMenu();return false;">禁止发言</a>');
            }else{
                ul.push('<a href="javascript:;" onclick="_mo.canSpeak(\'', u.uid,
                    '\');chat.closeUserMenu();return false;">恢复发言</a>');
            }
            if(ENV.roomType=='0') {
                if(u.usertype>'3') {
                    ul.push('<a href="javascript:;" onclick="_mo.addToAdminGroup(\'', u.uid,
                        '\');chat.closeUserMenu();return false;">设为管理</a>');
                }else{
                    ul.push('<a href="javascript:;" onclick="_mo.delFromAdminGroup(\'', u.uid,
                        '\');chat.closeUserMenu();return false;">取消管理</a>');
                }
            }
        }
        ul.push('</div>');
    ...
    createUserList: function (userlist) {
        ...
        for(var i=0, l=userlist.length; i<l; i++) {
            var user = userlist[i];
            var odd = (i%2==0)? 'odd': '';
            var curr = (chat.user.uid == user.uid && chat.user.type == 5)? 'current-user': '';
            var usertype = this.userTypes[user.usertype].className;
            var menu = (parseInt(chat.user.type)<=3 && parseInt(user.usertype)>parseInt(chat.user.type)  && chat.user.uid !=user.uid)? chat.makeUserMenu(user):'';
            var list = ['<tr><td class="', odd, '>'
            ,'"<div class="', curr, '" onmouseover="chat._showUserMenu(this);" onmouseout="chat._hideUserMenu(this);">'
            ,'<a href="javascript:;" class="', usertype, '" uid="', user.uid, '" '
            ,'onclick="chat.seluser(\'', user.uid, '\');return false;">'
            ,'<span>', user.username, '<br/>(', user.uid, ')</span>'
            ,'</a></div></td></tr>'];
            _ul.push(list.join(''));
        }
        _ul.push('</table>');
    ...
    refreshUserlist: function () {
        ...
        userlist.sort(this.sortByUserType);
        hs.$("onlineNum").innerHTML = userlist.length;
        hs.$("onlineUsers").innerHTML = this.createUserList(userlist);
    },
    ...
    refreshRecenlist: function () {
        ...
        for(var i in this._recentlist){
            ...
            tmp.push('<tr><td><a href="javascript:;" uid="', user.uid, '" username="', user.username,
                    '" onclick="chat.selRecentUser(this);return false;"><span>', user.username, '</span></a></td></tr>');
        }
        tmp.push('</table>');
    ...
    /*初始化消息客户端*/
    init: function () {
        var wait = navigator.userAgent.toLowerCase().indexOf('webkit')>-1? 1000: 0;
        setTimeout(function(){
...
function createFansList(data) {
    ...
    for(var i=0, l=data.length; i<l; i++) {
        ...
        rows.push(['<table class="fans-rank-item"><tr>',
                '<td class="col1" style="background:url(\/static2\/images\/xuhao', i+1,
            '.jpg) center center no-repeat;">&nbsp;</td>',
...
function showGiftBox(uid) {
    ...
    hs.$("hcoin").innerHTML = chat.huocoin;
}
        ...
        if(!_$("#face_box").attr('isLoaded')){
            ...
            for(var i=1; i<49; i++){
                facesHTML.push('<a href="javascript:;" onclick="chat.selectFace(\'', i, '\');return false;">',
                    '<img src="\/static2\/images\/face\/face', i, '.gif" width="24" height="24" \/>', '<\/a>');
            }
