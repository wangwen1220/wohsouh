...
//	var_dump($arr);
	...
	if (count($dbarr)>0){//绑定，则根据uid查得用户信息登录
		if (!empty($arr[uid])) {
			...
			$code_url = "/member.php?mod=logging&action=login&loginsubmit=yes&handlekey=login&floatlogin=yes&loginfield=username&username=".$login_data[0][username]."&questionid=0&answer=&isbound=isbound&social_session=".$_SESSION['Social_Login']."&session_uid=".$_SESSION['Social_Uid'];
