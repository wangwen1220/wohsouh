<tr>
	...
	<td>
		...
		<input type="text" name="author" class="suggest"
			width="300"
			url="?app=system&controller=template&action=suggestPhpwindUser&dsnid=<?=$dsnid?>&prefix=<?=urlencode($prefix)?>"
			allUrl="?app=system&controller=template&action=pagePhpwindUser&dsnid=<?=$dsnid?>&prefix=<?=urlencode($prefix)?>"
...
<tr>
	...
	<td>
		...
		<label><input name="special" value="2" type="checkbox"> 活动主题</label>
		<label><input name="special" value="3" type="checkbox"> 悬赏主题</label>
		<label><input name="special" value="4" type="checkbox"> 商品主题</label>
