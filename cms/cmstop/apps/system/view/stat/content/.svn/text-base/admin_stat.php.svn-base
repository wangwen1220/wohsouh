...
for ($y = date('Y'); ; $y--)
{
	...
	echo '<li><a href="javascript:;" onclick="$(\'#stat\').load(\'?app=system&controller=stat_content&action=admin_stat&userid='.$userid.'&year='.$y.'&month='.$month.'\');" '.$selected.'>'.$y.'年</a></li>';
	if ($i >= 10) break;
}
?>
</ul></div>
...
foreach ($months as $m)
{
	...
	echo '<a href="javascript:;" onclick="$(\'#stat\').load(\'?app=system&controller=stat_content&action=admin_stat&userid='.$userid.'&year='.$year.'&month='.$m.'\');" '.$selected.'>'.(substr($m, 0, 1) == 0 ? substr($m, 1) : $m).'月</a>';
}
?>
</div>
