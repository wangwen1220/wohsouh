...
$('#category_').tree({
	url:"?app=system&controller=stat_comment&action=cate&catid=%s",
	paramId : 'catid',
	...
	active : function(div, id, item){
		$('#stat').load('?app=system&controller=stat_comment&action=category_stat&catid='+id);
		current_catid = id;
	},
	prepared:function(){
		var t = this;
		$.getJSON('?app=system&controller=stat_comment&action=category_path&catid='+current_catid, function(path){
			t.open(path);
		});
	}
});
