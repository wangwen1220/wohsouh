...
<div class="tag_1">
   <div id="add_data">
	  <select id="changestat" style="width:100px;z-index:99;" marginTop="1" marginLeft="0">
  		  ...
  		  <option value="comment" selected>评论统计</option>
  		  <option value="pv">访问统计</option>
	  </select>
	</div>
	<ul class="tag_list" style="margin-left:160px">
	    <li><a href="?app=system&controller=stat_comment&action=index"<?if($action == 'index') echo 'class="s_3";'?>>综合统计</a></li>
	    <li><a href="?app=system&controller=stat_comment&action=category"<?if($action == 'category') echo 'class="s_3";'?>>按栏目统计</a></li>
	    <li><a href="?app=system&controller=stat_comment&action=model"<?if($action == 'model') echo 'class="s_3";'?>>按模型统计</a></li>
	    <li><a href="?app=system&controller=stat_comment&action=department"<?if($action == 'department') echo 'class="s_3";'?>>按部门统计</a></li>
	    <li><a href="?app=system&controller=stat_comment&action=admin"<?if($action == 'admin') echo 'class="s_3";'?>>按编辑统计</a></li>
	</ul>
...
$(function (){
	...
	function refresh(){
		$.get("?app=system&controller=stat_comment&action=refresh", function (data) {
			if(data == 1){
				location.reload();
			}
		});
